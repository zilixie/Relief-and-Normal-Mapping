...
vec3 bump_position(vec3 s) {
	float noise = 0.5 * perlin_noise(s);
	vec3 d = normalize(s);
...
void main() {
	...
	if (normal_type == 2) {
		...
		if (obj_id == 5) {
			...
			p = 300;
			mat3 texture = mat3(2,0,0,
							0,2,0,
							0,0,2);

			vec3 texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float orange_noise = (perlin_noise(texture_pos) + 1)/2;
			vec3 new_n = vec3(tes_vn[0], tes_vn[1], tes_vn[2]);

			if (orange_noise > 0.24) {
				kd = vec3(1.0, 0.50, 0.31);
			} 
			if (orange_noise > 0.28) {
				kd = vec3(1.0, 0.84, 0);
			} 
			if (orange_noise > 0.31) {
				kd = vec3(1.0, 0.65, 0);
			}
			if (orange_noise > 0.32) {
				kd = vec3(1.0, 0.55, 0);
			}
			if (orange_noise > 0.34) {
				kd = vec3(1.0, 0.45, 0);
			}
			if (orange_noise > 0.36) {
				kd = vec3(1.0, 0.25, 0);
			}
			if (orange_noise > 0.38) {
				kd = vec3(0.7, 0.13, 0.13);
			}
			if (orange_noise > 0.42) {
				kd = vec3(0.8, 0.3, 0.2);
			}
			...
			p = 500;
			mat3 texture = mat3(1,0,0,
							0,15,0,
							0,0,1);

			vec3 texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float green_noise = (perlin_noise(texture_pos) + 1)/2;
			vec3 new_n = vec3(tes_vn[0], tes_vn[1], tes_vn[2]);

			if (green_noise > 0.16) {
				kd = vec3(0.8, 0.98, 0.1);
			} 
			if (green_noise > 0.20) {
				kd = vec3(0.65, 0.98, 0.2);
			} 
			if (green_noise > 0.24) {
				kd = vec3(0.2, 0.80, 0.2);
			} 
			if (green_noise > 0.28) {
				kd = vec3(0.1, 0.9, 0.1);
			} 
			...
			if (green_noise > 0.32) {
				kd = vec3(0.0, 0.98, 0.6);
			}
			if (green_noise > 0.34) {
				kd = vec3(0.6, 0.98, 0);
			}
			if (green_noise > 0.36) {
				kd = vec3(0.18, 0.55, 0.34);
			}
			...
			vec3 texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float sand_noise = (perlin_noise(texture_pos) + 1)/2;
			vec3 new_n = vec3(tes_vn[0], tes_vn[1], tes_vn[2]);
			...
			if (sand_noise > 0.35) {
				kd = vec3(0.74, 0.72, 0.42);
			}
			mat3 bump = mat3(1,0,0,
							0,15,0,
							0,0,1);
			...
			vec3 tang = vec3(0,0,0);
			tangent(tes_vn, tang, bi_norm);
			new_n = cross(( bump_position(bump * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]) + eps*tang)
								- bump_position(bump * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2])) )/eps,
								 (bump_position(bump * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]) + eps*bi_norm)
								- bump_position(bump * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2])))/eps);
