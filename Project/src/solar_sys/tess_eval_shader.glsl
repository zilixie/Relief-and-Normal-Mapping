...
void main()
{
	...
	if (obj_id == 9) {
		...
		vec3 adjusted_noise_vec_3d = 0.2 * adjusted_noise_vec.xyz;

		gl_Position = vec4(vec3(tmp[0], tmp[1], tmp[2]) + adjusted_noise_vec_3d, tmp[3]);
		tes_pos = tmp;
	}
	if (obj_id == 8) {
		noise_color = vec3(0.6,0.985,0.6);
		vec3 noise_vec = vec3(0,0,0);
		vec4 adjusted_noise_vec = view * proj * camera * model * vec4(noise_vec, 1);
		vec3 adjusted_noise_vec_3d = 0.2 * adjusted_noise_vec.xyz;
		...
		noise_color = vec3(1.0,0.39,0.30);
		...
		mat3 texture = mat3(1,0,0,
					...
					0,0,1);
		vec3 texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
		float noise = 0.5 * (improved_perlin_noise(texture_pos) + 1)/2;
		vec3 noise_vec = vec3(0,0,0);

		if (noise > 0.14) {
			noise_color = vec3(0.41,0.41,0.41); //white


			texture = mat3(3,0,0,
						0,3,0,
						0,0,3);
			texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float height_noise = (improved_perlin_noise(texture_pos) + 1)/1.5;
			noise_vec = height_noise * tes_vn;
    	} 
		else {
			noise_color = vec3(0.41,0.41,0.41); //sea
			vec3 noise_vec = vec3(0,0,0);
		}
		...
		if (noise > 0.14) {
			noise_color = vec3(1.0,1.0,1.0); //white

			mat3 texture = mat3(2,0,0,
								0,2,0,
								0,0,2);
			vec3 texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float green_noise = (improved_perlin_noise(texture_pos) + 1)/2;
			if (green_noise <= 0.3) {
				noise_color = vec3(0.2,0.7,0.0); //forest
			}
			...
			noise_color = vec3(0.0,0.3,1.0); //sea

			texture = mat3(1.7,0,0,
						0,1.7,0,
						0,0,1.7);
			texture_pos = texture * vec3(tes_obj_space_pos[0], tes_obj_space_pos[1], tes_obj_space_pos[2]);
			float blue_noise = (improved_perlin_noise(texture_pos) + 1)/2;
			if (blue_noise >= 0.3) {
				noise_color = vec3(0.2,0.6,0.8);
			}
			vec3 noise_vec = vec3(0,0,0);
		}
